...
|---|---|---|---|---|
| A.1 Energy technology (transformation) root | `https://openenergyplatform.org/ontology/oeo/OEO_00020267` (primary), `https://openenergyplatform.org/ontology/oeo/OEO_00000011` (secondary) | "energy technology" (plan) + "energy converting component" (material) | **verified 2026-04-22 (imports/oeo-seed-verification.md)** | Architect ran `robot extract --method BOT` against both roots; combined subtree ~90 classes. Namespace corrected to HTTPS without hyphen. Original scope-stage A.1 candidate kept as dual secondary root. |
| A.2 Energy carrier root | `https://openenergyplatform.org/ontology/oeo/OEO_00020039` | "energy carrier" | **verified 2026-04-22** | Single root. Disposition OEO_00000151 deferred to V1. Namespace corrected to HTTPS without hyphen. |
| A.3 Aggregation type root | ~~`https://openenergyplatform.org/ontology/oeo/OEO_00140068`~~ (DROPPED) | "aggregation type" | **dropped 2026-04-22** | OEO v2.11.0 asserted subtree of 1 (just the root). Replaced by hand-seeded SKOS ConceptScheme at `modules/concept-schemes/aggregation-type.ttl` (sum / average / minimum / maximum / count) per ODP F.1. Curator to file upstream OEO issue. |
...
| A.5 Quantity-kind root | `https://openenergyplatform.org/ontology/oeo/OEO_00000350` | "quantity value" | **rejected (use QUDT)** | Note: OEO `_00000350` is "quantity *value*", semantically closer to an instance than a QUDT-style `qudt:QuantityKind`. **Recommendation: do not MIREOT this subtree; use `qudt:QuantityKind` directly.** See § D. Namespace corrected to HTTPS without hyphen. |
